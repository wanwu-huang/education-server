...
public class IntervalUtil {
     ...
     */
    public static boolean isInTheInterval (
            String data_value ,
            String interval ) {
        //将区间和data_value转化为可计算的表达式
        String formula = getFormulaByAllInterval ( data_value ,
                                                   interval ,
                                                   "||" );
     ...
     */
    private static String getFormulaByAllInterval (
            String date_value ,
            String interval ,
            String connector ) {
        ...
        for ( String limit : interval.split ( "U" ) ) {//如：（125%,135%）U (70%,80%)
            buff.append ( "(" )
                .append ( getFormulaByInterval ( date_value ,
                                                 limit ,
                                                 " && " ) )
                .append ( ")" )
                .append ( connector );
        ...
        allLimitInvel = allLimitInvel.substring ( 0 ,
                                                  index );
     ...
     * @param connector  连接符 如：&&
     */
    private static String getFormulaByInterval (
            String date_value ,
            String interval ,
            String connector ) {
        StringBuffer buff = new StringBuffer ( );
        ...
        for ( String halfInterval : interval.split ( "," ) ) {
            buff.append ( getFormulaByHalfInterval ( halfInterval ,
                                                     date_value ) )
                .append ( connector );
        }
        String limitInvel = buff.toString ( );
        int index = limitInvel.lastIndexOf ( connector );
        limitInvel = limitInvel.substring ( 0 ,
                                            index );
        return limitInvel;
    }
     ...
     */
    private static String getFormulaByHalfInterval (
            String halfInterval ,
            String date_value ) {
...
////		  <6
//		//结论区间
//		System.out.println ( IntervalUtil.isInTheInterval ( "10" , "[10,15)" ) );
//
//	}
