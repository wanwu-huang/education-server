...
public class SixModuleNextQuestion implements ModuleStrategy {
    ...
    public QuestionVO getNextQuestion(Long moduleId, int index, int moduleCount) {
        ...
        if (index >= size) {
            // 超过下标,查找下一量表  第7模块
            ModuleStrategy strategy = moduleStrategyFactory.getStrategy(7);
            return strategy.getNextQuestion(7L, 0, 1);
        }
