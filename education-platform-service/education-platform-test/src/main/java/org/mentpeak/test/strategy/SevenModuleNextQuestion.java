...
public class SevenModuleNextQuestion implements ModuleStrategy {
    ...
    public QuestionVO getNextQuestion(Long moduleId, int index, int moduleCount) {
        ...
        if (index >= size) {
            // 超过下标,查找下一量表  第8模块
            ModuleStrategy strategy = moduleStrategyFactory.getStrategy(8);
            return strategy.getNextQuestion(8L, 0, 1);
        }
