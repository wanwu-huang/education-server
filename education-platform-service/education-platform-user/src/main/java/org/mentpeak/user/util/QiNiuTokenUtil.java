package org.mentpeak.user.util;
...
public class QiNiuTokenUtil {
    ...
    private String templeteId = "templeteId";
    /**
     * 设置七牛云存储区域
     * Zone.zone0() ：华东
     * Zone.zone1() ：华北
...
//    // 静态加载配置文件中的数据
//     {
//        accessKey = SystemUtil.QN_AccessKey;
//        secretKey = SystemUtil.QN_SecretKey;
//        publicBucket = SystemUtil.QN_publicBucket;
//        privateBucket = SystemUtil.QN_privateBucket;
//        callbackURL = SystemUtil.QN_CallBackURL;
//        publicIMGURL = SystemUtil.QN_publicIMGURL;
//        priviteIMGURL = SystemUtil.QN_priviteIMGURL;
//    }
    ...
    private String getUrl (
            ...
            String key ) {
        if ( bocket == 0 ) {
            return getDownloadToken ( platformQiNiuProperties.getPriviteIMGURL ( ) + key );
        } else {
            return platformQiNiuProperties.getPublicIMGURL ( ) + key;
        }
    }


    public String getuplodTokenByKey ( String key ) {
        // 第一步： 先生成一个Auth验证
        Auth auth = Auth.create ( platformQiNiuProperties.getAccessKey ( ) ,
                                  platformQiNiuProperties.getSecretKey ( ) );
...
//        putPolicy.put("returnUrl","");
        ...
        // type : String
        putPolicy.put ( "callbackBodyType" ,
                        "application/json" );
        // 第三步： 生成一个上传凭证
        return auth.uploadToken ( platformQiNiuProperties.getPublicBucket ( ) ,
                                  key ,
                                  ( System.currentTimeMillis ( ) / 1000L + 3600 ) ,
                                  putPolicy );
    }
...
//        putPolicy.put("fileType",0);
    ...
    public String qiniuUploadFile (
        ...
        try {
            if ( null != fileFullName && ! "".equals ( fileFullName ) && new File ( fileFullName ).exists ( ) ) {
                ...
                boolean result = upload ( fileFullName ,
                                          key ,
                                          upToken );
                if ( result ) {
                    return platformQiNiuProperties.getPublicIMGURL ( ) + "/" + key;
    ...
    private boolean upload (
            ...
            String upToken ) {
        try {
            Response res = uploadManager.put ( filePath ,
                                               key ,
                                               upToken );
    ...
    public String uploadFile (
        ...
        if ( null != keyName && ! "".equals ( keyName ) ) {
            return qiniuUploadFile ( fileFullName ,
                                     keyPath ,
                                     getuplodTokenByKey ( keyName ) );
        } else {
            return qiniuUploadFile ( fileFullName ,
                                     keyPath ,
                                     getDefaultDynamicUpToken ( ) );
        }
    }
     ...
     * @return
     */
    public static Boolean sendMessageCheck (
            String accessKey ,
            String secretKey ,
            String[] phone ,
        ...
        try {
            Response resp = smsManager.sendMessage ( "1277906922204315648" ,
                                                     phone ,
                                                     map );
